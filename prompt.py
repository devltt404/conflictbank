...
def main(in_file, out_dir):
    ...
    with open(out_file, 'w', encoding='utf-8') as f:
        for data in all_datas:
            if len(data['temporal_conflict_time_span']) == 1:
                prompt = prompt_text + '\n\nEvidence: {0}\nQuestion: {1} in {2}\nA. {3}\nB. {4}\nC. {5}\nD. {6}\nAnswer:'.format(data['temporal_conflict_evidence'], data['question'], data['temporal_conflict_time_span'][0], data['options'][0], data['options'][1], data['options'][2], data['options'][3])
            else:
    ...
    with open(out_file, 'w', encoding='utf-8') as f:
        for data in all_datas:
            if len(data['temporal_conflict_time_span']) == 1:
                prompt = prompt_text + '\n\nEvidence: {0}\n{1}\nQuestion: {2} in {3}\nA. {4}\nB. {5}\nC. {6}\nD. {7}\nAnswer:'.format(data['default_evidence'], data['temporal_conflict_evidence'], data['question'], data['temporal_conflict_time_span'][0], data['options'][0], data['options'][1], data['options'][2], data['options'][3])
            else:
