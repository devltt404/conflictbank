...
def inference(model_name, input_dir, out_dir, logprobs):
    """
    ...
    llm = LLM(max_logprobs=logprobs * 2, model=model_name, dtype="float16", tensor_parallel_size=torch.cuda.device_count(), trust_remote_code=True)
    sampling_params = SamplingParams(logprobs=logprobs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logprobs', type=int, default=20, help="Number of log probabilities to request from the model.")
    args = parser.parse_args()

    inference(args.model_path, args.input_dir, args.out_dir, args.logprobs)